...
/////
///// JSON uses Arrays and Objects. These correspond here to the datatypes List<object> and Hashtable.
...
public class MiniJSON {
    ...
    /// <returns>An List<object>, a Hashtable, a double, a string, null, true, or false</returns>
    ...
    /// <summary>
    /// Converts a Hashtable / List<object> / Dictionary(string,string) object into a JSON string
    /// </summary>
    /// <param name="json">A Hashtable / List<object></param>
    ...
    protected static object parseNumber(char[] json, ref int index) {
        ...
        //Acies - добавил возможность создания Long-а
        bool isDouble = false;
        foreach (char ch in numberCharArray) {
            if (ch == '.') {
                isDouble = true;
                break;
            }
        ...
        if (isDouble) {
            ...
            return Int64.Parse(new string(numberCharArray)); // , CultureInfo.InvariantCulture);
        }
    ...
    protected static bool serializeValue(object value, StringBuilder builder) {
        ...
        if (value == null) {
            builder.Append("null");
        //Acies - пришлось временно убрать ЕTODO
        /*} else if (value.GetType().IsArray) {
            serializeArray(new List<object>((ICollection)value), builder);*/
        ...
        } else if (value.GetType().IsPrimitive && value is int || value is long) {
            serializeIntNumber(Convert.ToInt64(value), builder); //Acies добавил
    ...
    protected static void serializeIntNumber(long number, StringBuilder builder) {
        builder.Append(Convert.ToString(number)); // , CultureInfo.InvariantCulture));
    }
    #endregion
}
