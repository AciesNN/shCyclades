...
public class UIMapHexController : UIMapController {
	...
	public override GridPosition WorldPositionToCell(Vector3 pos) {
		...
		if (HexGridType == HexGridOrientation.VERTICAL && res.x % 2 != 0) {
			res.y = (int)System.Math.Floor((nPos.y - CellYSize * 0.5f) / CellYSize);
		} else if (HexGridType == HexGridOrientation.HORIZONTAL && res.y % 2 != 0) {
	...
	public override Vector3 CellToWorldPosition(GridPosition cell) {
		...
		float x = (cell.x + 0.5f + ( HexGridType == HexGridOrientation.HORIZONTAL && cell.y % 2 != 0 ? 0.5f : 0.0f )) * CellXSize;
		float y = (cell.y + 0.5f + ( HexGridType == HexGridOrientation.VERTICAL && cell.x % 2 != 0 ? 0.5f : 0.0f )) * CellYSize;
